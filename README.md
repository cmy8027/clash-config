...
| 策略组    | 类型       | 用途              |
| ------ | -------- | --------------- |
| 全部节点   | url-test | 地区组优先、包含所有节点并自动测试延迟 |
...
在规则模式和 `GLOBAL`、`PROXY` 等选择器中，脚本会保留现有地区分组的原名称，并把所有已生成的地区组整体排在候选列表最前面，不会因为测速结果变化而被普通节点挤到后面。订阅原有的 `GLOBAL`、`PROXY` 等选择器也会被保留。

1. 从 `HK香港` 开始的现有地区组；
2. `其他节点`；
3. `国外AI`、`YouTube`、`Pixiv`、`游戏专用`、`跟踪分析`、`广告过滤`、`谷歌服务`、`微软服务`、`Github`、`日本网站`、`下载软件`、`其他外网`；
4. `全部节点`、`默认节点`、`AI默认节点` 和其他未列入优先级的策略组。

未生成的地区组或服务组会自动跳过，不会影响其他策略组。

「全部节点」和「默认节点」的候选项也会按同样的逻辑排列：现有地区组在最前面，随后是订阅中的具体节点；地区组和节点仍由 `url-test` 统一测速和选择。
